            ...
            switch (ajax_request.status) {
                ...
                case 401:
                    ...
                    if (ms.error === 'Unauthorized') {
                        alerta("ERRO !!!",'Usuário não autorizado. Verifique os dados informados. Se o problema persistir entre em contato com o suporte',null);
                    } else {
...
function doPostToken(url, params, successCallBack) {
    ...
    ajax_request.setRequestHeader('Access-Control-Allow-Origin', '*');
    //ajax_request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    ajax_request.setRequestHeader("Authorization", gTokenApi.replace("Bearing",""));
